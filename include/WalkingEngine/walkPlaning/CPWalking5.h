...
namespace GaitManager
{
    ...
    public:
        ...
        void footprintPlan(int originStepNum);								//plan the footprint used to origin step
        void footprintPlan(double desx);									//plan the footprint used to forward or backward walk,desx(mm) relate to current ground world frame in x direction
        void footprintPlan(Eigen::Vector2d desP);							//plan the footprint used to walk to desired point,attitude no change,desP(mm) relate to current ground world frame
        void footprintPlan(Eigen::Vector2d desP, double phi);				//plan the footprint used to walk to desired point,attitude could change with phi(degree)--rotate around z axis,desP(mm) and phi relate to current ground world frame
        void footprintPlan(Eigen::Vector2d desP, bool phiOn);				//plan the footprint used to walk to desired point,attitude could change if phiOn is true,the phi((degree)--rotate around z axis) is determin by desP(1)/desP(0),desP(mm) and phi relate to current ground world frame
        void circleFprintPlan(Eigen::Vector2d desP, bool forwardOn = true); //plan the footprint used to walk around a circle--use desP as the center of the circle,desP(mm) relate to current ground world frame
        void originStep();
        void forward();
        void turnaround();
        void constructOneStep(Eigen::MatrixXd &fPrint, FootprintStatus fprintStatus, StanceStatus stanceSta, Eigen::Vector3d delxytheta, Eigen::Vector2d leftFootp, Eigen::Vector2d rightFootp, bool conflict);
        void nextFootprintPostion(StanceStatus stanceSta, Eigen::Vector3d delxytheta, Eigen::Vector2d &leftFootp, Eigen::Vector2d &rightFootp);
        void receiveGaitCommand();
        void receiveGaitCommand1();
        void commandRosInit();
        // void gaitCommandCallback(const std_msgs::Float64MultiArray::ConstPtr& gaitcomm);
        ...
        void receiveFootprint(Eigen::MatrixXd fPrint, double sT, FootprintStatus fprintStatus, StanceStatus stanceSta); //receive footprint and store in Footprint struct queue
        void walkPlan(Eigen::Vector2d desP, double phi);
        void initGaitParam(int fpNum, Eigen::MatrixXd fPrint, Eigen::VectorXd sT, StanceStatus stanceSta); //convert footprintPlan() and circleFprintPlan() to Footprint struct,ennable it to call updateThreeStep()
        void updateFrame(Eigen::MatrixXd &fPrint, int pNum, int fpcount);								   //update to next footprint frame
        void run();
        void updateThreeStep();																						   //update trajectory param according Footprint struct queue
        void updateThreeStep(Eigen::MatrixXd fPrint, double sT, FootprintStatus fprintStatus, StanceStatus stanceSta); //update three steps after every step
        void renewComAtStanceConvert(Eigen::Vector3d nextfPrint, Eigen::Vector3d &com, Eigen::Vector3d &comv);		   //renew com in current foot to next foot
        void convertTrajToNextStep(Eigen::Vector3d nextfPrint, Eigen::MatrixXd traj, int trajNum, Eigen::MatrixXd &trajInNextStep);
        ...
        void planCopTraj(Eigen::MatrixXd prefEnd, double stepTime, FootprintStatus fprintStatus); //derive reference cop end point from footprint,then fifth polynomial interpolation to derive cop reference trajectory
        double computeCpCoef(int j, double Ti, Eigen::Matrix2d &si, Eigen::Matrix2d &expi);
        void planCpTraj(Eigen::MatrixXd prefEnd, double stepTime, FootprintStatus fprintStatus); //derive capture point reference trajectory from cop reference trajectory
        void planCpTraj_NoDS(Eigen::MatrixXd prefEnd, double stepTime, FootprintStatus fprintStatus);
        void planComTraj(Eigen::MatrixXd cp_ref, double stepTime, FootprintStatus fprintStatus);
        void planComTraj_NoDS(Eigen::MatrixXd cp_ref, double stepTime, FootprintStatus fprintStatus);
        void footTrajectory(Eigen::MatrixXd fPrint, double stepTime, FootprintStatus fprintStatus, StanceStatus stanceSta); //foot position trajectory from footprint,use fifth polynomial interpolation
        void footTrajectory_NoDS(Eigen::MatrixXd fPrint, double stepTime, FootprintStatus fprintStatus, StanceStatus stanceSta);
        void replanFootTrajectory(Eigen::MatrixXd fPrint, double nextStepTime);
        void attitudeTrajectory(Eigen::MatrixXd fPrint, double stepTime, FootprintStatus fprintStatus, StanceStatus stanceSta); //foot and torso attitude trajectory from footprint,use Quaternion slerp interpolation
        void attitudeTrajectory_NoDS(Eigen::MatrixXd fPrint, double stepTime, FootprintStatus fprintStatus, StanceStatus stanceSta);
        Eigen::Vector2d convertVelocityToWorld(Eigen::Vector2d vel, Eigen::Vector3d fpInWorld);
        Eigen::Vector2d convertVecToWorld(Eigen::Vector2d vec, Eigen::Vector3d fpInWorld);
        void convertTrajToWorld(Eigen::MatrixXd traj, Eigen::MatrixXd &trajInWorld, Eigen::Vector3d fpInWorld);
        void convertCOMvToWorld(Eigen::MatrixXd COMv, Eigen::MatrixXd &COMvInWorld, Eigen::Vector3d fpInWorld);
        ...
        void trackTrajController(int count); //addmitance based zmp controller to track capture point reference trajectory
        Eigen::Vector6d fifthPolyInterpInit(Eigen::Vector6d bound, double t);
        double fifthPolyInterp(Eigen::Vector6d coef, double t);

        static CPWalking5 *GetInstance()
        {
        ...
        double coplen;	//coplen*2 eaqual cop move length at stance foot in x direction
        ...
        bool planningDone; //walk planing done flag,to void main.cpp thread to  run run() in doing walk planing
        bool onWalking;
        bool lastOnWalking;
        bool prepareStop;
        // bool recvOne;
        ...
        // Eigen::Vector4d observerProcessDeviation = Eigen::Vector4d(0.1, 0.1, 3, 3);//Eigen::Vector4d(0.1, 0.1, 3, 3);//
        //    Eigen::Vector2d observerMeasurementDeviation = Eigen::Vector2d(0.02,  0.02);//Vector2f(2,  2);//
        Eigen::Vector4d observerProcessDeviation = Eigen::Vector4d(0.01, 0.01, 0.03, 0.03); //Eigen::Vector4d(0.01, 0.01, 0.03, 0.03);//
        Eigen::Vector2d observerMeasurementDeviation = Eigen::Vector2d(0.02, 0.02);			//Vector2f(0.02,  0.02);//

        Eigen::MatrixXd referFrameInW; //use to renew reference frame in vrep

        Eigen::Vector3d nextFootPrint; //used to renew com in current foot to next foot

        Eigen::VectorXd jointVal; //lowpass the jointValue command for openloopController
